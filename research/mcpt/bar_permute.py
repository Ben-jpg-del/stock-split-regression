...
def get_permutation(
    ...
    # Validate start_index to ensure we have bars to permute
    if start_index >= n_bars - 1:
        raise ValueError(
            f"start_index ({start_index}) must be < n_bars - 1 ({n_bars - 1}). "
            f"Need at least 2 bars after start_index to permute."
        )
